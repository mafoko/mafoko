...
ENV GEOIP_BASE_URL      https://download.maxmind.com/app/geoip_download?edition_id=GeoLite2-Country&license_key=UifKrgTz7oqmj0nf&suffix=tar.gz
...
ADD ${GEOIP_BASE_URL} /tmp/
ADD ${GEOIP_BASE_URL} /tmp/

# unzip databases into database directory
#RUN mkdir -p ${GEOIP_DB_DIR} \
#&& gunzip -c /tmp/${GEOIP_CNTR_DB}.tar.gz > ${GEOIP_DB_DIR}/${GEOIP_CNTR_DB} \
#&& gunzip -c /tmp/${GEOIP_CITY_DB}.tar.gz > ${GEOIP_DB_DIR}/${GEOIP_CITY_DB} \
#&& rm -f /tmp/GeoLite2-*
